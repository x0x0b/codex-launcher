...
class CodexArgsBuilderTest : LightPlatformTestCase() {
    ...
    fun testComplexArgsFormattingOnNonWindows() {
        ...
        val result = CodexArgsBuilder.build(state, 11111, osProvider = osProvider, openAiApiKey = "dummy")
        ...
        assertEquals("""--api-key""", result[3])
        assertEquals('$' + """{OPENAI_API_KEY}""", result[4])
        assertEquals("""-c""", result[5])
        assertEquals("""'model_reasoning_effort=high'""", result[6])
        assertEquals("""-c""", result[7])
        assertEquals("""'notify=["curl", "-s", "-X", "POST", "http://localhost:11111/refresh", "-d"]'""", result[8])
        ...
        assertEquals("""-c""", result[13])
        assertEquals("""'mcp_servers.intellij.env={"IJ_MCP_SERVER_PORT"="64342"}'""", result[14])
    }
    ...
    fun testComplexArgsFormattingOnWindows() {
        ...
        val result = CodexArgsBuilder.build(state, 22222, osProvider = osProvider, openAiApiKey = "dummy")
        ...
        assertEquals("""'gpt-4o'""", result[2])
        assertEquals("""--api-key""", result[3])
        assertEquals('$' + """env:OPENAI_API_KEY""", result[4])
        assertEquals("""-c""", result[5])
        assertEquals("""model_reasoning_effort='high'""", result[6])
        assertEquals("""-c""", result[7])
        assertEquals("""notify='[\"curl\", \"-s\", \"-X\", \"POST\", \"http://localhost:22222/refresh\", \"-d\"]'""", result[8])
        assertEquals("""-c""", result[9])
        assertEquals(
    ...
    fun testComplexArgsFormattingOnWindowsWithPowerShell73OrOver() {
        ...
        val result = CodexArgsBuilder.build(state, 33333, osProvider = osProvider, openAiApiKey = "dummy")
        ...
        assertEquals("""model_reasoning_effort='high'""", result[6])
        assertEquals("""-c""", result[7])
        assertEquals("""notify='["curl", "-s", "-X", "POST", "http://localhost:33333/refresh", "-d"]'""", result[8])
        assertEquals("""-c""", result[9])
        assertEquals(
            """mcp_servers.intellij.command='C:\Program Files\JetBrains\IntelliJ IDEA Community Edition 2025.2.1\jbr\bin\java'""",
            result[10]
    ...
    fun testMinimalArgs() {
        ...
        val result = CodexArgsBuilder.build(state, null, osProvider = osProvider, openAiApiKey = "")
    ...
    fun testComplexArgsFormattingOnWindowsWithWSL() {
        ...
        val result = CodexArgsBuilder.build(state, 44444, osProvider = osProvider, openAiApiKey = "dummy")
